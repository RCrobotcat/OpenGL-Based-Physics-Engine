...
namespace RCNet
{
    namespace net
    {
        ...
        public:
            ...
            // asio::ip::tcp::v4() => ipv4 ip address
            ...
            void MessageClient(std::shared_ptr<connection<T>> client, const message<T> &msg)
            {
                ...
                } else
                {
                    ...
                    // 先用 std::remove 把要删的元素（client 或 nullptr）移动到容器末尾，再用 erase 真正从 deque 中删除它们
                    // std::remove 会把“等于指定值的元素”移动到容器末尾，并返回新的逻辑结尾迭代器，但它本身并不真正删除元素
                    // 相当于:
                    // auto newEnd = std::remove(begin, end, value); => 把要删的元素（value）移动到容器末尾 newEnd
                    // container.erase(newEnd, container.end()); => 删除 [newEnd, end) 区间的元素
                    m_deqConnections.erase(std::remove(m_deqConnections.begin(), m_deqConnections.end(), client),
                                           m_deqConnections.end());
                }
            }

            void MessageClient(uint32_t nClientID, const message<T> &msg)
            {
                // Iterate through all clients to find the correct one
                for (auto &client: m_deqConnections)
                {
                    if (client->GetID() == nClientID)
                    {
                        // Found the correct client, send the message
