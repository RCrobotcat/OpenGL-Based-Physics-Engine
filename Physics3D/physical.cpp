...
#pragma region structure
    ...
    Physical::Physical(RigidBody &&rigidBody, MotorizedPhysical *mainPhysical) : rigidBody(std::move(rigidBody)),
        mainPhysical(mainPhysical)
    {
    ...
    Physical::Physical(Physical &&other) noexcept : rigidBody(std::move(other.rigidBody)),
    ...
    ConnectedPhysical::ConnectedPhysical(RigidBody &&rigidBody, Physical *parent,
                                         HardPhysicalConnection &&connectionToParent) : Physical(std::move(rigidBody),
        parent->mainPhysical), parent(parent), connectionToParent(std::move(connectionToParent))
    {
    }

    ConnectedPhysical::ConnectedPhysical(Physical &&phys, Physical *parent,
                                         HardPhysicalConnection &&connectionToParent) : Physical(std::move(phys)),
        parent(parent), connectionToParent(std::move(connectionToParent))
    {
    }

    ConnectedPhysical::ConnectedPhysical(Physical &&phys, Physical *parent, HardConstraint *constraintWithParent,
                                         const CFrame &attachOnThis,
                                         const CFrame &attachOnParent) : Physical(std::move(phys)), parent(parent),
                                                                         connectionToParent(
                                                                             std::unique_ptr<HardConstraint>(
                                                                                 constraintWithParent), attachOnThis,
                                                                             attachOnParent)
    {
    }
    ...
    void Physical::makeMainPart(AttachedPart &newMainPart)
    {
        ...
        {
            connectedPhys.connectionToParent.attachOnParent = newCenterCFrame.globalToLocal(
                connectedPhys.connectionToParent.attachOnParent);
        }
        if (!isMainPhysical())
        {
            ConnectedPhysical *self = (ConnectedPhysical *) this;
            self->connectionToParent.attachOnChild = newCenterCFrame.globalToLocal(
                self->connectionToParent.attachOnChild);
        }
    }
    ...
    */
    ...
    void Physical::attachPhysical(Physical *phys, HardConstraint *constraint, const CFrame &attachToThis,
                                  const CFrame &attachToThat)
    {
    ...
    void Physical::attachPhysical(MotorizedPhysical *phys, HardConstraint *constraint, const CFrame &attachToThis,
                                  const CFrame &attachToThat)
    {
    ...
    void Physical::attachPart(Part *part, HardConstraint *constraint, const CFrame &attachToThis,
                              const CFrame &attachToThat)
    {
        ...
        } else
        {
            ...
            childPhysicals.push_back(ConnectedPhysical(Physical(part, this->mainPhysical), this, constraint,
                                                       attachToThat, attachToThis));
    ...
    void Physical::attachPhysical(MotorizedPhysical *phys, const CFrame &attachment)
    {
        ...
        for (ConnectedPhysical &conPhys: phys->childPhysicals)
        {
            ...
            conPhysOnThis.connectionToParent.attachOnParent = attachment.localToGlobal(
                conPhysOnThis.connectionToParent.attachOnParent);
    ...
    void Physical::detachAllChildPhysicals()
    {
        ...
        // calls the destructors on all (now invalid) children, deleting the constraints in the process
    }
    ...
    static void computeInternalRelativeMotionTree(MonotonicTreeBuilder<RelativeMotion> &builder,
                                                  MonotonicTreeNode<RelativeMotion> &curNode,
                                                  const ConnectedPhysical &conPhys,
                                                  const RelativeMotion &motionOfParent)
    {
        ...
        } else
        {
            ...
            {
                computeInternalRelativeMotionTree(builder, curNode.children[i], conPhys.childPhysicals[i],
                                                  motionOfSelf);
            }
        }
    }

    InternalMotionTree MotorizedPhysical::getInternalRelativeMotionTree(
        UnmanagedArray<MonotonicTreeNode<RelativeMotion>> &&mem) const noexcept
    {
        ...
        for (std::size_t i = 0; i < mainSize; i++)
        {
            ...
            } else
            {
                ...
                {
                    computeInternalRelativeMotionTree(builder, curNode.children[j], conPhys.childPhysicals[j],
                                                      motionOfPrimallyAttached);
                }
            }
        }
    ...
    COMMotionTree MotorizedPhysical::getCOMMotionTree(
        UnmanagedArray<MonotonicTreeNode<RelativeMotion>> &&mem) const noexcept
    {
    ...
    void Physical::detachPart(Part *part)
    {
        ...
        if (rigidBody.getPartCount() == 1)
        {
            ...
            MotorizedPhysical *mainPhys = this->mainPhysical;
            // save main physical because it'll get deleted by parent->detachChild()
            if (this != mainPhys)
            {
                ...
                // double move, but okay, since remove really only needs the address of self
...
#pragma region getters
    ...
    */
    ...
    double MotorizedPhysical::getInertiaOfPointInDirectionLocal(const Vec3Local &localPoint,
                                                                const Vec3Local &localDirection) const
    {
        SymmetricMat3 accMat = getResponseMatrix(localPoint);

        Vec3 force = localDirection;
        Vec3 accel = accMat * force;
        // 向量投影：取加速度在力(localDirection)的方向上的分量
        // 并且除以力(localDirection)的大小
        double accelInForceDir = accel * localDirection / lengthSquared(localDirection);

        return 1 / accelInForceDir; // m = F/a, so 1/(a/F) = m
    ...
    double MotorizedPhysical::getInertiaOfPointInDirectionRelative(const Vec3Relative &relPoint,
                                                                   const Vec3Relative &relDirection) const
    {
        return getInertiaOfPointInDirectionLocal(getCFrame().relativeToLocal(relPoint),
                                                 getCFrame().relativeToLocal(relDirection));
    }
    ...
    Motion ConnectedPhysical::getMotion() const
    {
        ...
        RelativeMotion inGlobalFrame = motionBetweenParentAndChild.extendBegin(
            CFrame(parent->getCFrame().getRotation()));
    ...
    std::tuple<double, Vec3, TranslationalMotion> InternalMotionTree::getInternalMotionOfCenterOfMass() const
    {
        ...
        motorPhys->mutuallyRecurse(relativeMotionTree,
                                   [&totalMass, &totalCenterOfMass, &totalMotion](
                               const MonotonicTreeNode<RelativeMotion> &node, const ConnectedPhysical &conPhys)
                                   {
    ...
    COMMotionTree InternalMotionTree::normalizeCenterOfMass() &&
    {
        ...
        return COMMotionTree(this->motorPhys, std::move(this->relativeMotionTree), std::get<0>(m), std::get<1>(m),
                             std::get<2>(m));
    }

    SymmetricMat3 COMMotionTree::getInertia() const
    {
        SymmetricMat3 totalInertia = getTranslatedInertiaAroundCenterOfMass(
            motorPhys->rigidBody.inertia, motorPhys->rigidBody.mass, this->mainCOMOffset);
        motorPhys->mutuallyRecurse(relativeMotionTree,
                                   [&totalInertia](const MonotonicTreeNode<RelativeMotion> &node,
                                                   const ConnectedPhysical &conPhys)
                                   {
                                       totalInertia += getTransformedInertiaAroundCenterOfMass(
                                           conPhys.rigidBody.inertia, conPhys.rigidBody.mass,
                                           node.value.locationOfRelativeMotion);
                                   });
    ...
    {
        FullTaylor<SymmetricMat3> totalInertia = getTranslatedInertiaDerivativesAroundCenterOfMass(
            motorPhys->rigidBody.inertia, motorPhys->rigidBody.mass, this->mainCOMOffset, -this->motionOfCenterOfMass);
        motorPhys->mutuallyRecurse(relativeMotionTree,
                                   [&totalInertia](const MonotonicTreeNode<RelativeMotion> &node,
                                                   const ConnectedPhysical &conPhys)
                                   {
                                       totalInertia += getTransformedInertiaDerivativesAroundCenterOfMass(
                                           conPhys.rigidBody.inertia, conPhys.rigidBody.mass,
                                           node.value.locationOfRelativeMotion, node.value.relativeMotion);
                                   });
    ...
    Motion COMMotionTree::getMotion() const
    {
        ...
        return -motionOfCom + this->motorPhys->motionOfCenterOfMass.getMotionOfPoint(
                   cf.localToRelative(-this->centerOfMass));
    }

    Vec3 COMMotionTree::getInternalAngularMomentum() const
    {
        Vec3 velocityOfMain = -motionOfCenterOfMass.getVelocity();
        Vec3 totalAngularMomentum = getAngularMomentumFromOffsetOnlyVelocity(
            this->mainCOMOffset, velocityOfMain, motorPhys->rigidBody.mass);

        motorPhys->mutuallyRecurse(relativeMotionTree,
                                   [&totalAngularMomentum](const MonotonicTreeNode<RelativeMotion> &node,
                                                           const ConnectedPhysical &conPhys)
                                   {
                                       const RelativeMotion &relMotion = node.value;
                                       SymmetricMat3 relativeInertia = relMotion.locationOfRelativeMotion.getRotation().
                                               localToGlobal(conPhys.rigidBody.inertia);
