...
public:
    ...
    ~ArrayMesh()
    {
        // Note: in a real application proper copy/move constructors should be used to avoid double deletion
        // For simplicity we will assume one owner or leak resources if copied. The move operators below help.
        /*
    ...
    ArrayMesh(ArrayMesh &&other) noexcept : vertexArrayID(other.vertexArrayID),
...
public:
    ...
    {
        int index = findKnownShapeClassIndex(shapeClass);
        ...
        ArrayMesh arMesh(reinterpret_cast<float *>(vertexBuf), reinterpret_cast<float *>(normalBuf),
                         shapeClassPolyhedron.triangleCount);
        delete[] vertexBuf;
        delete[] normalBuf;
...
public:
    ...
    CustomPart(const Shape &shape, const GlobalCFrame &position, const PartProperties &properties, int meshIndex,
               Color color) : Part(shape, position, properties), meshIndex(meshIndex), color(color)
    {
    }

    CustomPart(const Shape &shape, const GlobalCFrame &position, const PartProperties &properties,
               Color color) : Part(shape, position, properties),
                              meshIndex(meshRegistry.getMeshIndexFor(shape.baseShape.get())), color(color)
    {
    }
};
...
int main(int argc, const char **argv)
{
    ...
    // Create a cube that will fall
    std::unique_ptr<CustomPart> fallingCube = std::make_unique<CustomPart>(
        boxShape(1.0, 1.0, 1.0),
        GlobalCFrame(0.0, 10.0, 0.0),
        basicProperties,
        Colors::RED);
    ...
    std::unique_ptr<DirectionalGravity> gravity = std::make_unique<DirectionalGravity>(Vec3(0.0, -9.81, 0.0));
