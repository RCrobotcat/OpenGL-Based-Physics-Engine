...
namespace P3D
{
    ...
    constexpr auto operator%(const Vector<T1, 2> &first,
                             const Vector<T2, 2> &second) noexcept -> decltype(
    ...
    constexpr auto operator%(const Vector<T1, 3> &first,
                             const Vector<T2, 3> &second) noexcept -> Vector<decltype(
        first[1] * second[2] - first[2] * second[1]), 3>
    {
    ...
    */
    template<typename T, size_t Size>
    constexpr Vector<T, Size> projectToPlaneNormal(const Vector<T, Size> &vec,
                                                   const Vector<T, Size> &planeNormal) noexcept
    {
