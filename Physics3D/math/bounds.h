...
namespace P3D
{
    ...
    struct BoundsTemplate
    {
        ...
        constexpr BoundsTemplate(const PositionTemplate<T> &min,
                                 const PositionTemplate<T> &max) noexcept : min(min), max(max)
        {
        }
        ...
        {
            return BoundsTemplate(min - Vector<T, 3>(amount, amount, amount),
                                  max + Vector<T, 3>(amount, amount, amount));
        }
        ...
        {
            Vector<T, 3> diag = max - min;
            return diag.x * diag.y * diag.z;
        }
    };

    // AABB intersection test
    // broad phase collision detection core algorithm
    template<typename T>
    ...
    constexpr BoundsTemplate<PT> operator+(const BoundingBoxTemplate<VT> &localBox,
                                           const PositionTemplate<PT> &pos) noexcept
    {
