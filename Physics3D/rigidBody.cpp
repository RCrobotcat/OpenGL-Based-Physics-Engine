...
namespace P3D
{
    RigidBody::RigidBody(Part *mainPart) : mainPart(mainPart),
    ...
    CFrame RigidBody::makeMainPart(AttachedPart &newMainPart)
    {
        if (liesInVector(parts, &newMainPart))
        {
            ...
            newMainPart.attachment = ~newCenterCFrame;
            // since this CFrame is now the attachment of the former mainPart, we can just invert the original CFrame
    ...
    void RigidBody::refreshWithNewParts()
    {
        ...
        SymmetricMat3 totalInertia = getTranslatedInertiaAroundCenterOfMass(
            mainPart->getInertia(), mainPart->getMass(), mainPart->getLocalCenterOfMass() - totalCenterOfMass);;

        for (const AttachedPart &p: parts)
        {
            const Part *part = p.part;
            CFrame attachRelativeToCOM = CFrame(p.attachment.getPosition() - totalCenterOfMass,
                                                p.attachment.getRotation());
            totalInertia += getTransformedInertiaAroundCenterOfMass(part->getInertia(), part->getMass(),
                                                                    part->getLocalCenterOfMass(), attachRelativeToCOM);
        }
        this->mass = totalMass;
