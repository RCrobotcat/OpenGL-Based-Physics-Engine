...
namespace P3D
{
    void DirectionalGravity::apply(WorldPrototype *world)
    {
        ...
        {
            p->applyForceAtCenterOfMass(gravity * p->totalMass); // gravity: g, mass: m, G = m * g
        }
    }
    ...
    double DirectionalGravity::getPotentialEnergyForObject(const WorldPrototype *world,
                                                           const MotorizedPhysical &phys) const
    {
