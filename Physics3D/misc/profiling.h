...
namespace P3D
{
    ...
    public:
        ...
        inline HistoricTally(char const *const labels[static_cast<size_t>(Category::COUNT)], size_t size) : history(
            size)
        {
    ...
    public:
        ...
        inline BreakdownAverageProfiler(char const *const labels[static_cast<size_t>(ProcessType::COUNT)],
                                        size_t capacity) : HistoricTally<std::chrono::nanoseconds, ProcessType>(
                                                               labels, capacity), tickHistory(capacity)
        {
        }
        ...
        inline void mark(ProcessType process, ProcessType overrideOldProcess)
        {
            ...
            if (currentProcess != static_cast<ProcessType>(-1))
            {
                HistoricTally<std::chrono::nanoseconds, ProcessType>::addToTally(
                    overrideOldProcess, curTime - startTime);
            }
            startTime = curTime;
            currentProcess = process;
        }
