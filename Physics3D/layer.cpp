...
namespace P3D
{
    ...
    WorldLayer::WorldLayer(WorldLayer &&other) noexcept : tree(std::move(other.tree)),
                                                          parent(other.parent)
    {
    ...
    static void addMotorPhysToGroup(BoundsTree<Part> &tree, MotorizedPhysical *phys, Part *group)
    {
        ...
        modifyGroupRecursive(alloc, base.first, base.second, group, group->getBounds(),
                             [&](TreeNodeRef &groupNode, const BoundsTemplate<float> &groupNodeBounds)
                             {
                                 ...
                                 {
                                     addRecursive(alloc, groupNode.asTrunk(), groupNode.getTrunkSize(), TreeNodeRef(mp),
                                                  mp->getBounds());
                                 }
                                 ...
                                 {
                                     curTrunkSize = addRecursive(alloc, curTrunk, curTrunkSize, TreeNodeRef(&part),
                                                                 part.getBounds());
                                 });
    ...
    {
        return &knownLayers[id / ColissionLayer::NUMBER_OF_SUBLAYERS].subLayers[
            id % ColissionLayer::NUMBER_OF_SUBLAYERS];
    }
    ...
    ColissionLayer::ColissionLayer() : world(nullptr), collidesInternally(true),
                                       subLayers{WorldLayer(this), WorldLayer(this)}
    {
    }

    ColissionLayer::ColissionLayer(WorldPrototype *world, bool collidesInternally) : world(world),
        collidesInternally(collidesInternally), subLayers{WorldLayer(this), WorldLayer(this)}
    {
    }

    ColissionLayer::ColissionLayer(ColissionLayer &&other) noexcept : world(other.world),
                                                                      collidesInternally(other.collidesInternally),
                                                                      subLayers{
                                                                          std::move(other.subLayers[0]),
                                                                          std::move(other.subLayers[1])
                                                                      }
    {
        other.world = nullptr;
    ...
    {
        return std::abs(sphereCenter.x) > scale[0] + sphereRadius || std::abs(sphereCenter.y) > scale[1] + sphereRadius
               || std::abs(sphereCenter.z) > scale[2] + sphereRadius;
    }
    ...
    static void findColissionsBetween(std::vector<Colission> &colissions, const BoundsTree<Part> &treeA,
                                      const BoundsTree<Part> &treeB)
    {
