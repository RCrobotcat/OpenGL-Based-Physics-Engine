...
#pragma endregion
    ...
    static void createNodeFor(P3D::BoundsTree<Part> &tree, MotorizedPhysical *phys)
    {
        ...
        } else
        {
            ...
            {
                newNodeSize = P3D::addRecursive(alloc, *newNode, newNodeSize, P3D::TreeNodeRef(static_cast<void *>(&p)),
                                                p.getBounds());
            });
    ...
    static void createNewNodeFor(MotorizedPhysical *motorPhys, BoundsTree<Part> &layer, Part *repPart)
    {
        ...
        } else
        {
            ...
            motorPhys->forEachPart([&alloc, &newNode, &newNodeSize, &layer, repPart](Part &p)
            {
                if (&p.layer->tree == &layer)
                {
                    newNodeSize = addRecursive(alloc, *newNode, newNodeSize, TreeNodeRef(static_cast<void *>(&p)),
                                               p.getBounds());
                }
            });
    ...
    static void assignLayersForPhysicalRecurse(const Physical &phys,
                                               std::vector<std::pair<WorldLayer *, std::vector<const Part *>>> &
                                               foundLayers)
    {
    ...
    void WorldPrototype::notifyPhysicalHasBeenSplit(const MotorizedPhysical *mainPhysical,
                                                    MotorizedPhysical *newlySplitPhysical)
    {
