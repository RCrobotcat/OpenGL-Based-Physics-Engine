...
namespace P3D
{
    ...
    public:
        ...
        TreeNodeRef &operator=(TreeNodeRef &&other) noexcept
        {
            this->ptr = other.ptr;
            other.ptr = INVALID_REF;
            return *this;
        }
#else
    ...
    TreeNodeRef::TreeNodeRef(TreeTrunk *trunk, int trunkSize, bool isGroupHead) noexcept : ptr(
        reinterpret_cast<std::uintptr_t>(trunk) | (static_cast<std::uintptr_t>(trunkSize) - 1) | (
            isGroupHead ? BRANCH_FACTOR : 0))
    {
        assert(trunkSize >= 2 && trunkSize <= BRANCH_FACTOR); // trunkSize must be between 2-BRANCH_FACTOR
        assert((reinterpret_cast<std::uintptr_t>(trunk) & (BRANCH_FACTOR * sizeof(int64_t) - 1)) == 0);
        // check trunk is aligned correctly
    }
    ...
    struct TrunkSIMDHelperFallback
    {
        ...
        static std::array<bool, BRANCH_FACTOR> getAllContainsBounds(const TreeTrunk &trunk,
                                                                    const BoundsTemplate<float> &boundsToContain);

        static std::array<float, BRANCH_FACTOR> computeAllCosts(const TreeTrunk &trunk);

        static std::array<float, BRANCH_FACTOR> computeAllCombinationCosts(
            const BoundsArray<BRANCH_FACTOR> &boundsArr, const BoundsTemplate<float> &boundsExtention);

        static std::pair<int, int> computeFurthestObjects(const BoundsArray<BRANCH_FACTOR * 2> &boundsArray, int size);

        static int getLowestCombinationCost(const TreeTrunk &trunk, const BoundsTemplate<float> &boundsExtention,
                                            int nodeSize);

        static std::array<bool, BRANCH_FACTOR> computeOverlapsWith(const TreeTrunk &trunk, int trunkSize,
                                                                   const BoundsTemplate<float> &bounds);

        // indexed result[a][b]
        static OverlapMatrix computeBoundsOverlapMatrix(const TreeTrunk &trunkA, int trunkASize,
                                                        const TreeTrunk &trunkB, int trunkBSize);

        static OverlapMatrix computeBoundsOverlapMatrixAVX(const TreeTrunk &trunkA, int trunkASize,
                                                           const TreeTrunk &trunkB, int trunkBSize);

        static OverlapMatrix computeBoundsOverlapMatrixSSE(const TreeTrunk &trunkA, int trunkASize,
                                                           const TreeTrunk &trunkB, int trunkBSize);

        // indexed result[i][j] with j >= i+1
        static OverlapMatrix computeInternalBoundsOverlapMatrix(const TreeTrunk &trunk, int trunkSize);

        static std::array<float, BRANCH_FACTOR> computeAllExtentionCosts(
            const TreeTrunk &trunk, int trunkSize, const BoundsTemplate<float> &extraBounds);

        // returns resulting destTrunk size
        static int transferNodes(TreeTrunk &srcTrunk, int srcTrunkStart, int srcTrunkEnd, TreeTrunk &destTrunk,
                                 int destTrunkSize);

        static BoundsArray<BRANCH_FACTOR * 2> combineBoundsArrays(const TreeTrunk &trunkA, int trunkASize,
                                                                  const TreeTrunk &trunkB, int trunkBSize);
    ...
    int addRecursive(TrunkAllocator &allocator, TreeTrunk &curTrunk, int curTrunkSize, TreeNodeRef &&newNode,
                     const BoundsTemplate<float> &bounds);

    int removeRecursive(TrunkAllocator &allocator, TreeTrunk &curTrunk, int curTrunkSize, const void *objectToRemove,
                        const BoundsTemplate<float> &bounds);

    bool containsObjectRecursive(const TreeTrunk &trunk, int trunkSize, const void *object,
                                 const BoundsTemplate<float> &bounds);

    const TreeNodeRef *getGroupRecursive(const TreeTrunk &curTrunk, int curTrunkSize, const void *groupRepresentative,
                                         const BoundsTemplate<float> &representativeBounds);
    ...
    */
    ...
    inline bool modifyGroupRecursive(TrunkAllocator &allocator, TreeTrunk &curTrunk, int curTrunkSize,
                                     const void *groupRepresentative,
                                     const BoundsTemplate<float> &groupRepresentativeBounds, const Func &modification)
    {
        assert(curTrunkSize >= 0 && curTrunkSize <= BRANCH_FACTOR);
        std::array<bool, BRANCH_FACTOR> couldContain = TrunkSIMDHelperFallback::getAllContainsBounds(
            curTrunk, groupRepresentativeBounds);
        for (int i = 0; i < curTrunkSize; i++)
        {
            ...
            if (subNode.isTrunkNode())
            {
                ...
                {
                    if (modifyGroupRecursive(allocator, subTrunk, subTrunkSize, groupRepresentative,
                                             groupRepresentativeBounds, modification))
                    {
    ...
    */
    template<typename Func>
    inline int contractGroupRecursive(TrunkAllocator &allocator, TreeTrunk &curTrunk, int curTrunkSize,
                                      const void *groupRepresentative,
                                      const BoundsTemplate<float> &groupRepresentativeBounds, const Func &modification)
    {
        assert(curTrunkSize >= 0 && curTrunkSize <= BRANCH_FACTOR);
        ...
        for (int i = 0; i < curTrunkSize; i++)
        {
            ...
            if (subNode.isTrunkNode())
            {
                ...
                if (subNode.isGroupHead())
                {
                    if (containsObjectRecursive(subTrunk, subTrunkSize, groupRepresentative, groupRepresentativeBounds))
                    {
                        std::optional<BoundsTemplate<float>> newTrunkBounds = modification(
                            subNode, curTrunk.getBoundsOfSubNode(i));
                ...
                {
                    int newSubSize = contractGroupRecursive(allocator, subTrunk, subTrunkSize, groupRepresentative,
                                                            groupRepresentativeBounds, modification);
                    if (newSubSize != -1)
                    {
                        assert(newSubSize >= 1);
                        ...
                        } else
                        {
                            ...
                            curTrunk.setBoundsOfSubNode(
                                i, TrunkSIMDHelperFallback::getTotalBounds(subTrunk, newSubSize));
                        }
                        return curTrunkSize;
                    }
                }
    ...
    inline void forEachFilteredRecurse(const TreeTrunk &curTrunk, int curTrunkSize, const Filter &filter,
                                       const Func &func)
    {
        ...
        for (int i = 0; i < curTrunkSize; i++)
        {
            if (passes[i])
            {
                ...
                if (subNode.isTrunkNode())
                {
                    forEachFilteredRecurse<Boundable, Filter, Func>(subNode.asTrunk(), subNode.getTrunkSize(), filter,
                                                                    func);
                } else
                {
                    func(*static_cast<Boundable *>(subNode.asObject()));
    ...
    void forEachColissionWithRecursive(Boundable *obj, const BoundsTemplate<float> &objBounds, const TreeTrunk &trunk,
                                       int trunkSize, const Func &func)
    {
        ...
        for (int i = 0; i < trunkSize; i++)
        {
            ...
            if (subNode.isTrunkNode())
            {
                forEachColissionWithRecursive<Boundable, SIMDHelper, Func>(
                    obj, objBounds, subNode.asTrunk(), subNode.getTrunkSize(), func);
            } else
            {
    ...
    // Calls the given function for each pair of leaf nodes from the two trunks
    template<typename Boundable, typename SIMDHelper, typename Func>
    void forEachColissionWithRecursive(const TreeTrunk &trunk, int trunkSize, Boundable *obj,
                                       const BoundsTemplate<float> &objBounds, const Func &func)
    {
        std::array<bool, BRANCH_FACTOR> collidesWith = SIMDHelper::computeOverlapsWith(trunk, trunkSize, objBounds);
        ...
        for (int i = 0; i < trunkSize; i++)
        {
            ...
            {
                forEachColissionWithRecursive<Boundable, SIMDHelper, Func>(
                    subNode.asTrunk(), subNode.getTrunkSize(), obj, objBounds, func);
            } else
            {
    ...
    // Calls the given function for each pair of leaf nodes from the two trunks
    template<typename Boundable, typename SIMDHelper, typename Func>
    void forEachColissionBetweenRecursive(const TreeTrunk &trunkA, int trunkASize, const TreeTrunk &trunkB,
                                          int trunkBSize, const Func &func)
    {
            ...
            for (int b = 0; b < trunkBSize; b++)
            {
                ...
                if (aIsTrunk)
                {
                    if (bIsTrunk)
                    {
                        // both trunk nodes
                        // split both
                        forEachColissionBetweenRecursive<Boundable, SIMDHelper, Func>(
                            aNode.asTrunk(), aNode.getTrunkSize(), bNode.asTrunk(), bNode.getTrunkSize(), func);
                    } else
                    {
                        // a is trunk, b is not
                        // split a
                        forEachColissionWithRecursive<Boundable, SIMDHelper, Func>(
                            aNode.asTrunk(), aNode.getTrunkSize(), static_cast<Boundable *>(bNode.asObject()),
                            trunkB.getBoundsOfSubNode(b), func);
                    }
                } else
                {
                    if (bIsTrunk)
                    {
                        ...
                        // split b
                        forEachColissionWithRecursive<Boundable, SIMDHelper, Func>(
                            static_cast<Boundable *>(aNode.asObject()), trunkA.getBoundsOfSubNode(a), bNode.asTrunk(),
                            bNode.getTrunkSize(), func);
                    } else
                    {
                        // both leaf nodes
    ...
    void forEachColissionInternalRecursive(const TreeTrunk &curTrunk, int curTrunkSize, const Func &func)
    {
            ...
            for (int b = a + 1; b < curTrunkSize; b++)
            {
                ...
                if (aIsTrunk)
                {
                    ...
                    } else
                    {
                        ...
                        forEachColissionWithRecursive<Boundable, SIMDHelper, Func>(
                            aNode.asTrunk(), aNode.getTrunkSize(), static_cast<Boundable *>(bNode.asObject()),
                            curTrunk.getBoundsOfSubNode(b), func);
                    }
                } else
                {
                    if (bIsTrunk)
                    {
                        ...
                        // split b
                        forEachColissionWithRecursive<Boundable, SIMDHelper, Func>(
                            static_cast<Boundable *>(aNode.asObject()), curTrunk.getBoundsOfSubNode(a), bNode.asTrunk(),
                            bNode.getTrunkSize(), func);
                    } else
                    {
                        // both leaf nodes
        ...
        for (int i = 0; i < curTrunkSize; i++)
        {
            ...
            {
                forEachColissionInternalRecursive<Boundable, SIMDHelper, Func>(
                    subNode.asTrunk(), subNode.getTrunkSize(), func);
            }
        }
    }
    ...
    public:
        ...
        FilteredTreeIteratorPrototype(const TreeTrunk &baseTrunk, int baseTrunkSize,
                                      const Filter &filter) : trunkStack(), filter(filter)
        {
            if (baseTrunkSize == 0) return;
    ...
    public:
        ...
        inline BoundsTreePrototype(BoundsTreePrototype &&other) noexcept : baseTrunk(std::move(other.baseTrunk)),
                                                                           baseTrunkSize(other.baseTrunkSize),
        ...
        void addToGroup(void *newObject, const BoundsTemplate<float> &newObjectBounds, const void *groupRepresentative,
                        const BoundsTemplate<float> &groupRepBounds);

        void mergeGroups(const void *groupRepA, const BoundsTemplate<float> &repABounds, const void *groupRepB,
                         const BoundsTemplate<float> &repBBounds);

        void transferGroupTo(const void *groupRep, const BoundsTemplate<float> &groupRepBounds,
                             BoundsTreePrototype &destinationTree);

        void updateObjectBounds(const void *object, const BoundsTemplate<float> &originalBounds,
                                const BoundsTemplate<float> &newBounds);
        ...
        bool groupContains(const void *groupRep, const BoundsTemplate<float> &groupRepBounds, const void *object,
                           const BoundsTemplate<float> &bounds) const;
        ...
        void removeSubGroup(GroupIter iter, const GroupIterEnd &iterEnd)
        {
            ...
            {
                int newMainSize = contractGroupRecursive(this->allocator, this->baseTrunk, this->baseTrunkSize,
                                                         firstObject.first, firstObject.second,
                                                         [this, &iter, &iterEnd, &firstObject](
                                                     TreeNodeRef &group, const BoundsTemplate<float> &groupNodeBounds)
                                                         {
                                                             if (!group.isTrunkNode())
                                                             {
                                                                 throw
                                                                         "Attempting to remove more than one object from a single-object group!";
                                                             }
                                                             TreeTrunk &groupTrunk = group.asTrunk();
                                                             int groupTrunkSize = group.getTrunkSize();

                                                             groupTrunkSize = removeRecursive(
                                                                 this->allocator, groupTrunk, groupTrunkSize,
                                                                 firstObject.first, firstObject.second);
                                                             assert(groupTrunkSize != -1); // should have been verified

                                                             do
                                                             {
                                                                 std::pair<const void *, BoundsTemplate<float>>
                                                                         currentlyMoving = *iter;
                                                                 groupTrunkSize = removeRecursive(
                                                                     this->allocator, groupTrunk, groupTrunkSize,
                                                                     currentlyMoving.first, currentlyMoving.second);
                                                                 if (groupTrunkSize == -1)
                                                                 {
                                                                     throw
                                                                             "Iterator provided an object which could not be found in this group!";
                                                                 }
                                                                 ++iter;
                                                             } while (iter != iterEnd);
                                                             ...
                                                             if (groupTrunkSize >= 2)
                                                             {
                                                                 ...
                                                                 return std::optional<BoundsTemplate<float>>(
                                                                     TrunkSIMDHelperFallback::getTotalBounds(
                                                                         groupTrunk, groupTrunkSize));
                                                             } else if (groupTrunkSize == 1)
                                                             {
                                                                 BoundsTemplate<float> resultingBounds = groupTrunk.
                                                                         getBoundsOfSubNode(0);
                                                                 group = std::move(groupTrunk.subNodes[0]);
                                                                 if (group.isTrunkNode())
                                                                 {
                                                                     group.makeGroupHead();
                                                                     // make sure to preserve the group head if the trunk is overwritten
                                                                 }
        ...
        void transferSplitGroupTo(GroupIter iter, const GroupIterEnd &iterEnd, BoundsTreePrototype &destinationTree)
        {
            ...
            if (iter != iterEnd)
            {
                ...
                do
                {
                    ...
                    newGroupTrunkSize = addRecursive(destinationTree.allocator, *newGroupTrunk, newGroupTrunkSize,
                                                     TreeNodeRef(const_cast<void *>(obj.first)), obj.second);
                    ++iter;
                } while (iter != iterEnd);

                destinationTree.baseTrunkSize = addRecursive(destinationTree.allocator, destinationTree.baseTrunk,
                                                             destinationTree.baseTrunkSize,
                                                             TreeNodeRef(newGroupTrunk, newGroupTrunkSize, true),
                                                             TrunkSIMDHelperFallback::getTotalBounds(
                                                                 *newGroupTrunk, newGroupTrunkSize));
            } else
            {
        ...
        {
            return IteratorFactoryWithEnd<FilteredTreeIteratorPrototype<Filter>>{
                FilteredTreeIteratorPrototype<Filter>(baseTrunk, baseTrunkSize, filter)
            };
        }

        // unsafe functions
        inline std::pair<TreeTrunk &, int> getBaseTrunk()
        {
            return std::pair<TreeTrunk &, int>(this->baseTrunk, this->baseTrunkSize);
        }

        inline std::pair<const TreeTrunk &, int> getBaseTrunk() const
        {
            return std::pair<const TreeTrunk &, int>(this->baseTrunk, this->baseTrunkSize);
        }
    ...
    template<typename Boundable>
    bool updateGroupBoundsRecursive(TreeTrunk &curTrunk, int curTrunkSize, const Boundable *groupRep,
                                    const BoundsTemplate<float> &originalGroupRepBounds)
    {
        assert(curTrunkSize >= 0 && curTrunkSize <= BRANCH_FACTOR);
        std::array<bool, BRANCH_FACTOR> couldContain = TrunkSIMDHelperFallback::getAllContainsBounds(
            curTrunk, originalGroupRepBounds);
        for (int i = 0; i < curTrunkSize; i++)
        {
            if (!couldContain[i]) continue;
    ...
    public:
        ...
        BoundableCastIterator &operator++()
        {
            ++iter;
            return *this;
        }
    ...
    public:
        ...
        struct BoundableIteratorAdapter
        {
            ...
            std::pair<const void *, BoundsTemplate<float>> operator*() const
            {
                ...
                return std::pair<const void *, BoundsTemplate<float>>(static_cast<const void *>(item),
                                                                      item->getBounds());
            }
        ...
        {
            tree.addToGroup(static_cast<void *>(newObject), newObject->getBounds(),
                            static_cast<const void *>(groupRepresentative), groupRepresentative->getBounds());
        }
        ...
        {
            if (!(iter != iterEnd)) return;
            modifyGroupRecursive(tree.allocator, tree.baseTrunk, tree.baseTrunkSize, groupRep, groupRep->getBounds(),
                                 [this, &iter, &iterEnd](TreeNodeRef &groupNode, BoundsTemplate<float> groupBounds)
                                 {
                                     ...
                                     while (iter != iterEnd)
                                     {
                                         ...
                                         curTrunkSize = addRecursive(this->tree.allocator, trunk, curTrunkSize,
                                                                     TreeNodeRef(curObj), curObjBounds);
        ...
        {
            tree.mergeGroups(static_cast<const void *>(groupRepA), groupRepA->getBounds(),
                             static_cast<const void *>(groupRepB), groupRepB->getBounds());
        }
        ...
        bool groupContains(const Boundable *groupRep, const Boundable *object) const
        {
            ...
            return tree.groupContains(static_cast<const void *>(groupRep), groupRep->getBounds(),
                                      static_cast<const void *>(object), object->getBounds());
        }
        ...
        {
            bool success = updateGroupBoundsRecursive<Boundable>(tree.baseTrunk, tree.baseTrunkSize, groupRep,
                                                                 originalGroupRepBounds);
        ...
        {
            tree.transferSplitGroupTo(BoundableIteratorAdapter<GroupIter>{std::move(iter)}, iterEnd,
        ...
        {
            forEachFilteredRecurse<Boundable, Filter, Func>(this->tree.baseTrunk, this->tree.baseTrunkSize, filter,
                                                            func);
        }

        // expects a function of the form void(Boundable& object)
        template<typename Func>
        void forEachInGroup(const void *groupRepresentative, const BoundsTemplate<float> &groupRepBounds,
                            const Func &func) const
        {
            const TreeNodeRef *group = getGroupRecursive(this->tree.baseTrunk, this->tree.baseTrunkSize,
                                                         groupRepresentative, groupRepBounds);
            if (group == nullptr)
            {
        ...
        BoundableCastIterator<Boundable, BoundsTreeIteratorPrototype> begin() const
        {
            return BoundableCastIterator<Boundable, BoundsTreeIteratorPrototype>(this->tree.begin());
        }

        IteratorEnd end() const { return IteratorEnd(); }

        template<typename Filter>
        IteratorFactoryWithEnd<BoundableCastIterator<Boundable, FilteredTreeIteratorPrototype<Filter>>> iterFiltered(
        ...
        void forEachColission(const Func &func) const
        {
            ...
            forEachColissionInternalRecursive<Boundable, TrunkSIMDHelperFallback, Func>(
                this->tree.baseTrunk, this->tree.baseTrunkSize, func);
        }
        ...
        void forEachColissionWith(const BoundsTree &other, const Func &func) const
        {
            ...
            forEachColissionBetweenRecursive<Boundable, TrunkSIMDHelperFallback, Func>(
                this->tree.baseTrunk, this->tree.baseTrunkSize, other.tree.baseTrunk, other.tree.baseTrunkSize, func);
        }
