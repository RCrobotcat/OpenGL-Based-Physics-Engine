...
namespace P3D
{
    ...
    std::array<bool, BRANCH_FACTOR> TrunkSIMDHelperFallback::getAllContainsBounds(
        const TreeTrunk &trunk, const BoundsTemplate<float> &boundsToContain)
    {
    ...
    std::array<float, BRANCH_FACTOR> TrunkSIMDHelperFallback::computeAllCombinationCosts(
    ...
    std::pair<int, int> TrunkSIMDHelperFallback::computeFurthestObjects(
        const BoundsArray<BRANCH_FACTOR * 2> &boundsArray, int size)
    {
    ...
    int TrunkSIMDHelperFallback::getLowestCombinationCost(const TreeTrunk &trunk,
                                                          const BoundsTemplate<float> &boundsExtention, int nodeSize)
    {
        std::array<float, BRANCH_FACTOR> costs = TrunkSIMDHelperFallback::computeAllCombinationCosts(
            trunk.subNodeBounds, boundsExtention);
    ...
    std::array<bool, BRANCH_FACTOR> TrunkSIMDHelperFallback::computeOverlapsWith(
        const TreeTrunk &trunk, int trunkSize, const BoundsTemplate<float> &bounds)
    {
    ...
    OverlapMatrix TrunkSIMDHelperFallback::computeBoundsOverlapMatrix(const TreeTrunk &trunkA, int trunkASize,
    ...
    std::array<float, BRANCH_FACTOR> TrunkSIMDHelperFallback::computeAllExtentionCosts(
    ...
    int TrunkSIMDHelperFallback::transferNodes(TreeTrunk &srcTrunk, int srcTrunkStart, int srcTrunkEnd,
                                               TreeTrunk &destTrunk, int destTrunkSize)
    {
    ...
    BoundsArray<BRANCH_FACTOR * 2> TrunkSIMDHelperFallback::combineBoundsArrays(
        const TreeTrunk &trunkA, int trunkASize, const TreeTrunk &trunkB, int trunkBSize)
    {
    ...
    bool TrunkSIMDHelperFallback::exchangeNodesBetween(TreeTrunk &trunkA, int &trunkASize, TreeTrunk &trunkB,
                                                       int &trunkBSize)
    {
        ...
        BoundsArray<BRANCH_FACTOR * 2> allBounds = TrunkSIMDHelperFallback::combineBoundsArrays(
            trunkA, trunkASize, trunkB, trunkBSize);
        ...
        assert(aResultSize >= 2 && bResultSize >= 2);
        // Guaranteed by totalSize >= BRANCH_FACTOR + 2 && aResultSize <= BRANCH_FACTOR && bResultSize <= BRANCH_FACTOR

        // checks to see if a change needs to be made
        bool a0ComesFromA = aResult[0] < trunkASize;
        bool b0ComesFromA = bResult[0] < trunkASize;
        if (a0ComesFromA != b0ComesFromA)
        {
                     ...
                     const BoundsTemplate<float> &bounds)
    {
        ...
        if (curTrunkSize == BRANCH_FACTOR)
        {
            ...
            {
                int newSize = addRecursive(allocator, chosen.asTrunk(), chosen.getTrunkSize(), std::move(newNode),
                                           bounds);
                        ...
                        const BoundsTemplate<float> &bounds)
    {
        ...
        for (int i = 0; i < curTrunkSize; i++)
        {
            ...
            if (subNode.isTrunkNode())
            {
                ...
                int newSubNodeTrunkSize = removeRecursive(allocator, subNodeTrunk, subNode.getTrunkSize(),
                                                          objectToRemove, bounds);
                ...
                if (newSubNodeTrunkSize != -1)
                {
                    if (newSubNodeTrunkSize == 1)
                    {
                        ...
                        {
                            subNode.makeGroupHead();
                            // in the rare case that it removes Z in a node that is a group like this: GH[TN[X,Y],Z]. Without this the grouping would be lost
                        }
                        allocator.freeTrunk(&subNodeTrunk);
                    } else
                    {
                        curTrunk.setBoundsOfSubNode(
                            i, TrunkSIMDHelperFallback::getTotalBounds(subNodeTrunk, newSubNodeTrunkSize));
    ...
    struct TreeGrab
    {
        ...
        TreeGrab(int resultingGroupSize, TreeNodeRef &&nodeRef,
                 const BoundsTemplate<float> &nodeBounds) : resultingGroupSize(resultingGroupSize),
    ...
    // returns new trunkSize if removed, -1 if not removed
    static TreeGrab grabGroupRecursive(TrunkAllocator &allocator, TreeTrunk &curTrunk, int curTrunkSize,
                                       ...
                                       const BoundsTemplate<float> &representativeBounds)
    {
        ...
        for (int i = 0; i < curTrunkSize; i++)
        {
            ...
            if (subNode.isTrunkNode())
            {
                ...
                {
                    // try
                    TreeGrab recursiveResult = grabGroupRecursive(allocator, subNodeTrunk, subNode.getTrunkSize(),
                                                                  groupRepresentative, representativeBounds);
                    ...
                    if (newSubNodeTrunkSize != -1)
                    {
                        ...
                        {
                            // remove reduntant trunk
                            curTrunk.setSubNode(i, std::move(subNodeTrunk.subNodes[0]),
                                                subNodeTrunk.getBoundsOfSubNode(0));
                            allocator.freeTrunk(&subNodeTrunk);
                        } else
                        {
                            curTrunk.setBoundsOfSubNode(
                                         ...
                                         const BoundsTemplate<float> &representativeBounds)
    {
        ...
        for (int i = 0; i < curTrunkSize; i++)
        {
            ...
            if (subNode.isTrunkNode())
            {
                ...
                {
                    const TreeNodeRef *recursiveFound = getGroupRecursive(
                        subNodeTrunk, subTrunkSize, groupRepresentative, representativeBounds);
    ...
    static bool insertGroupIntoGroup(TrunkAllocator &sourceAlloc, TrunkAllocator &destinationAlloc,
                                     TreeTrunk &baseTrunk, int baseTrunkSize, const void *groupToAddToRep,
                                     const BoundsTemplate<float> &groupToAddToRepBounds, TreeNodeRef &&groupToDestroy,
                                     const BoundsTemplate<float> &groupToDestroyBounds)
    {
        bool groupWasFound = modifyGroupRecursive(destinationAlloc, baseTrunk, baseTrunkSize, groupToAddToRep,
                                                  groupToAddToRepBounds,
                                                  [&sourceAlloc, &destinationAlloc, &groupToDestroy, &
                                                      groupToDestroyBounds](
                                              TreeNodeRef &group, const BoundsTemplate<float> &groupBounds)
                                                  {
                                                      TreeTrunk *trunk;
                                                      int trunkSize;
                                                      if (group.isLeafNode())
                                                      ...
                                                      if (groupToDestroy.isTrunkNode())
                                                      {
                                                          forEachRecurseWithBounds(
                                                              groupToDestroy.asTrunk(), groupToDestroy.getTrunkSize(),
                                                              [&](void *object, const BoundsTemplate<float> &bounds)
                                                              {
                                                                  trunkSize = addRecursive(
                                                                      destinationAlloc, *trunk, trunkSize,
                                                                      TreeNodeRef(object), bounds);
                                                              });
                                                      } else
                                                      {
                                                          trunkSize = addRecursive(
                                                              destinationAlloc, *trunk, trunkSize,
                                                              TreeNodeRef(groupToDestroy.asObject()),
                                                              groupToDestroyBounds);
                                                      }
                                                      ...
                                                      if (groupToDestroy.isTrunkNode())
                                                      {
                                                          ...
                                                          freeTrunksRecursive(
                                                              sourceAlloc, destroyTrunk, groupToDestroy.getTrunkSize());
                                                      }
    ...
    {
        this->baseTrunkSize = addRecursive(allocator, baseTrunk, baseTrunkSize,
                                           TreeNodeRef(newTrunk, newTrunkSize, true),
                                           TrunkSIMDHelperFallback::getTotalBounds(*newTrunk, newTrunkSize));
    }
    ...
    void BoundsTreePrototype::addToGroup(void *newObject, const BoundsTemplate<float> &newObjectBounds,
                                         const void *groupRepresentative, const BoundsTemplate<float> &groupRepBounds)
    {
        bool foundGroup = modifyGroupRecursive(allocator, baseTrunk, baseTrunkSize, groupRepresentative, groupRepBounds,
                                               [this, newObject, &newObjectBounds](
                                           TreeNodeRef &groupNode, const BoundsTemplate<float> &groupNodeBounds)
                                               {
                                                   assert(groupNode.isGroupHeadOrLeaf());
                                                   if (groupNode.isTrunkNode())
                                                   {
                                                       TreeTrunk &groupTrunk = groupNode.asTrunk();
                                                       int resultingSize = addRecursive(
                                                           allocator, groupTrunk, groupNode.getTrunkSize(),
                                                           TreeNodeRef(newObject), newObjectBounds);
                                                       groupNode.setTrunkSize(resultingSize);
                                                       return TrunkSIMDHelperFallback::getTotalBounds(
                                                           groupTrunk, resultingSize);
                                                   } else
                                                   {
                                                       TreeTrunk *newTrunkNode = allocator.allocTrunk();
                                                       newTrunkNode->setSubNode(
                                                           0, std::move(groupNode), groupNodeBounds);
                                                       newTrunkNode->setSubNode(
                                                           1, TreeNodeRef(newObject), newObjectBounds);
    ...
    void BoundsTreePrototype::mergeGroups(const void *groupRepA, const BoundsTemplate<float> &repABounds,
                                          const void *groupRepB, const BoundsTemplate<float> &repBBounds)
    {
        TreeGrab grabbed = grabGroupRecursive(this->allocator, this->baseTrunk, this->baseTrunkSize, groupRepA,
                                              repABounds);
        ...
        bool groupBWasFound = insertGroupIntoGroup(this->allocator, this->allocator, this->baseTrunk,
                                                   this->baseTrunkSize, groupRepB, repBBounds,
                                                   std::move(grabbed.nodeRef), grabbed.nodeBounds);
    ...
    void BoundsTreePrototype::transferGroupTo(const void *groupRep, const BoundsTemplate<float> &groupRepBounds,
                                              BoundsTreePrototype &destinationTree)
    {
        TreeGrab grabbed = grabGroupRecursive(this->allocator, this->baseTrunk, this->baseTrunkSize, groupRep,
                                              groupRepBounds);
        if (grabbed.resultingGroupSize == -1)
        {
        ...
        bool groupBWasFound = addRecursive(destinationTree.allocator, destinationTree.baseTrunk,
                                           destinationTree.baseTrunkSize, std::move(grabbed.nodeRef),
                                           grabbed.nodeBounds);
    ...
    bool BoundsTreePrototype::groupContains(const void *object, const BoundsTemplate<float> &bounds,
                                            const void *groupRep, const BoundsTemplate<float> &groupRepBounds) const
    {
        const TreeNodeRef *groupFound = getGroupRecursive(this->baseTrunk, this->baseTrunkSize, groupRep,
                                                          groupRepBounds);
        if (!groupFound)
        {
            throw "Group not found!";
    ...
    static bool updateObjectBoundsRecurive(TreeTrunk &curTrunk, int curTrunkSize, const void *object,
                                           const BoundsTemplate<float> &originalBounds,
                                           const BoundsTemplate<float> &newBounds)
    {
        assert(curTrunkSize >= 0 && curTrunkSize <= BRANCH_FACTOR);
        std::array<bool, BRANCH_FACTOR> couldContain = TrunkSIMDHelperFallback::getAllContainsBounds(
            curTrunk, originalBounds);
        for (int i = 0; i < curTrunkSize; i++)
        {
            if (!couldContain[i]) continue;
    ...
    void BoundsTreePrototype::updateObjectBounds(const void *object, const BoundsTemplate<float> &originalBounds,
                                                 const BoundsTemplate<float> &newBounds)
    {
        bool wasFound = updateObjectBoundsRecurive(this->baseTrunk, this->baseTrunkSize, object, originalBounds,
                                                   newBounds);
        if (!wasFound) throw "Object was not found!";
    }

    static bool findAndReplaceObjectRecursive(TreeTrunk &curTrunk, int curTrunkSize, const void *oldObject,
                                              void *newObject, const BoundsTemplate<float> &bounds)
    {
        assert(curTrunkSize >= 0 && curTrunkSize <= BRANCH_FACTOR);
        std::array<bool, BRANCH_FACTOR> couldContain = TrunkSIMDHelperFallback::getAllContainsBounds(curTrunk, bounds);
        for (int i = 0; i < curTrunkSize; i++)
    ...
    void BoundsTreePrototype::findAndReplaceObject(const void *oldObject, void *newObject,
                                                   const BoundsTemplate<float> &bounds)
    {
    ...
    static bool disbandGroupRecursive(TreeTrunk &curTrunk, int curTrunkSize, const void *groupRep,
                                      const BoundsTemplate<float> &groupRepBounds)
    {
        assert(curTrunkSize >= 0 && curTrunkSize <= BRANCH_FACTOR);
        std::array<bool, BRANCH_FACTOR> couldContain = TrunkSIMDHelperFallback::getAllContainsBounds(
            curTrunk, groupRepBounds);
        for (int i = 0; i < curTrunkSize; i++)
        {
            if (!couldContain[i]) continue;
        ...
        for (int subNodeAI = 0; subNodeAI < trunkSize - 1; subNodeAI++)
        {
            ...
            for (int subNodeBI = subNodeAI + 1; subNodeBI < trunkSize - 1; subNodeBI++)
            {
                ...
                if (subTrunkSizeA + subTrunkSizeB >= BRANCH_FACTOR + 2)
                {
                    ...
                    if (TrunkSIMDHelperFallback::exchangeNodesBetween(subTrunkA, subTrunkSizeA, subTrunkB,
                                                                      subTrunkSizeB))
                    {
                        // update treenoderefs
                        ...
                        trunk.setBoundsOfSubNode(
                            subNodeAI, TrunkSIMDHelperFallback::getTotalBounds(subTrunkA, subTrunkSizeA));
                        trunk.setBoundsOfSubNode(
                            subNodeBI, TrunkSIMDHelperFallback::getTotalBounds(subTrunkB, subTrunkSizeB));
                ...
                } else
                {
                    ...
                    subTrunkSizeA = TrunkSIMDHelperFallback::transferNodes(
                        subTrunkB, 1, subTrunkSizeB, subTrunkA, subTrunkSizeA);
                    subNodeA.setTrunkSize(subTrunkSizeA);
                    trunk.setBoundsOfSubNode(
                        subNodeAI, TrunkSIMDHelperFallback::getTotalBounds(subTrunkA, subTrunkSizeA));
    ...
    inline static void swapNodesBetweenTrunks(TreeTrunk &trunkA, int indexInTrunkA, TreeTrunk &trunkB,
                                              int indexInTrunkB)
    {
    ...
    static void improveTrunkVertical(TreeTrunk &trunk)
    {
        ...
        {
            if (subTrunkI == smallestSubNodeI) continue;
            // the smallest node cannot contain a node larger than itself, also, trying to swap these would store this node in itself, leading to memory leak and objects disappearing
        ...
        if (largestItemI != -1)
        {
            ...
            trunk.setBoundsOfSubNode(largestItemSubTrunkI,
                                     TrunkSIMDHelperFallback::getTotalBounds(chosenSubTrunk, chosenSubTrunkSize));
        }
    }
    ...
    static int moveElementsOutOfGroup(TrunkAllocator &alloc, TreeTrunk &trunk, int trunkSize)
    {
        ...
        for (int subNodeI = 0; subNodeI < trunkSize; subNodeI++)
        {
            ...
            // find a node, and try to swap it with an element from a group
            if (!subNode.isGroupHeadOrLeaf())
            {
                // no breaking up groups
                TreeTrunk &subTrunk = subNode.asTrunk();
                int subTrunkSize = subNode.getTrunkSize();
                ...
                } else
                {
                    ...
                    int resultingSize = TrunkSIMDHelperFallback::transferNodes(
                        subTrunk, resultingSubTrunkSize, subTrunkSize, trunk, trunkSize);
                    subNode.setTrunkSize(resultingSubTrunkSize);
                    trunk.setBoundsOfSubNode(
                        subNodeI, TrunkSIMDHelperFallback::getTotalBounds(subTrunk, resultingSubTrunkSize));
