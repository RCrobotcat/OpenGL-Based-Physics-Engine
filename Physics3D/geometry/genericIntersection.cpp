...
namespace P3D
{
    ...
    {
        Vec3f furthest1 = info.scaleFirst * info.first.furthestInDirection(info.scaleFirst * searchDirection);
        // in local space of first
        Vec3f transformedSearchDirection = -info.transform.relativeToLocal(searchDirection);
        Vec3f furthest2 = info.scaleSecond * info.second.furthestInDirection(
                              info.scaleSecond * transformedSearchDirection); // in local space of second
    ...
    bool runEPATransformed(const ColissionPair &info, const Tetrahedron &s, Vec3f &intersection, Vec3f &exitVector,
                           ComputationBuffers &bufs)
    {
        ...
        for (int iter = 0; iter < EPA_MAX_ITER; iter++)
        {
            ...
            } else // 四面体已经足够包住原点了，EPA 结束，计算交点和穿透向量
            {
                ...
                MinkowskiPointIndices inds[3]{
                    bufs.knownVecs[closestTriangle[0]], bufs.knownVecs[closestTriangle[1]],
